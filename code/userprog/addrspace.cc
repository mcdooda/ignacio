...
void AddrSpace::InitBitMap() {
    ...
    if (!AllocateStackPages(NbPagesStack - 1, ThreadNbPages)) {
        printf("\n\nGROS PROBLEME Init\n\n");
    }
}
//TODO Est-ce que l'argument nPages est vraiment nécessaire ?

int AddrSpace::AllocateStackPages(int stackBottom, int nPages) {
    int i;
    for (i = 0; i < nPages; i++) {
        if (bm->Test(stackBottom - i)) {
            return 0;
        }
    }
    for (i = 0; i < nPages; i++) {
...
int AddrSpace::GetNextFreeStack() {
    ...
    for (i = 1; i < NbPagesStack; i += ThreadNbPages) {
        if (!bm->Test(NbPagesStack - i)) {
            if (!AllocateStackPages(NbPagesStack - i, ThreadNbPages)) {
                printf("\n\nGROS PROBLEME NextFree\n\n");
                return -1;
            } else {
